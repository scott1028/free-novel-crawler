...
class Quanben5NovelGrabber(NovelGrabber):
    def get_title_reg(self):
        reg_title = re.compile(
            r'<div.*?class="topbar">.*?<h1>(?P<title>.*?)</h1>', re.DOTALL
        )
        return reg_title

    def get_article_area_reg(self):
        reg_article = re.compile(
            '<div.*?class="wrapper".*?>(?P<article>.*?)<div class="footer".*?>',
            re.DOTALL,
        )
        return reg_article
    ...
    def get_chapter_urls_reg(self):
        reg_url = re.compile(
            '<li.*?class="c3">.*?<a href="(?P<url>.*?)".*?>.*?</li>', re.MULTILINE
        )
        return reg_url
    ...
    def get_novel_content_reg(self):
        reg_content = re.compile(
            '<div.*?id="content".*?>(?P<content>.*?)<div.*?class="nlist_page"',
            re.DOTALL,
        )
        return reg_content
