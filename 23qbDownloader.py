...
class X23QBNovelGrabber(NovelGrabber):
    def get_title_reg(self):
        reg_title = re.compile(
            r'<div class="d_title">.*?<h1>(?P<title>.*?)</h1>', re.DOTALL
        )
        return reg_title

    def get_article_area_reg(self):
        reg_article = re.compile(
            '<ul.*?id="chapterList".*?>.*?(?P<article>.*?)</ul', re.DOTALL
        )
        return reg_article

    def get_chapter_urls_reg(self):
        reg_url = re.compile('<li><a.*?href.*?="(?P<url>.*?)">.*?</li>', re.MULTILINE)
        return reg_url
    ...
    def get_novel_content_reg(self):
        reg_content = re.compile(
            '<div id="TextContent".*?>(?P<content>.*?)<div.*?class="bd"', re.DOTALL
        )
        return reg_content


if __name__ == "__main__":
    X23QBNovelGrabber(TXTENCODE="gbk", tip="ex: https://www.23qb.com/book/788/").run()
