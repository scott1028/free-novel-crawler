...
class EightBookNovelGrabber(NovelGrabber):
    ...
    def get_article_area_reg(self):
        reg_article = re.compile(
            '<div.*?class="subtitles".*?>(?P<article>.*?)</div>', re.DOTALL
        )
        return reg_article
    ...
    def get_novel_content_reg(self):
        reg_content = re.compile(
            '<div.*?id="text".*?>(?P<content>.*?)<div.*?', re.DOTALL
        )
        return reg_content
...
if __name__ == "__main__":
    EightBookNovelGrabber(
        TXTENCODE="utf-8", tip="ex: https://8book.com/novelbooks/138546/"
    ).run()
