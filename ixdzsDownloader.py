...
class IxdzsNovelGrabber(NovelGrabber):
    def get_title_reg(self):
        reg_title = re.compile(
            r'<div.*?class="d_info".*?>.*?<h1>(?P<title>.*?)</h1>', re.DOTALL
        )
        return reg_title

    def get_article_area_reg(self):
        reg_article = re.compile(
            r'<div.*?id="i-chapter".*?>.*?(?P<article>.*?)</div>', re.DOTALL
        )
        return reg_article

    def get_chapter_urls_reg(self):
        reg_url = re.compile(
            r'<li.*?><a.*?href.*?="(?P<url>.*?)".*?>.*?</li>', re.MULTILINE
        )
        return reg_url
    ...
    def get_novel_content_reg(self):
        reg_content = re.compile(
            r'<div.*?class.*?=.*?"content".*?>(?P<content>.*?)<div.*?class="line".*?>',
            re.DOTALL,
        )
        return reg_content
