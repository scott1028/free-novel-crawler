...
class TsnwbNovelGrabber(NovelGrabber):
    def get_title_reg(self):
        reg_title = re.compile(
            r'<div.*?id="info".*?>.*?<h1>(?P<title>.*?)</h1>', re.DOTALL
        )
        return reg_title

    def get_article_area_reg(self):
        reg_article = re.compile(
            '<div.*?id="list".*?>.*?<ul.*?class="chapters".*?>.*?(?P<article>.*?)</ul>',
            re.DOTALL,
        )
        return reg_article
    ...
    def get_chapter_urls_reg(self):
        reg_url = re.compile(
            '<li.*?class="chapter".*?><a.*?href.*?="(?P<url>.*?)".*?>.*?</li>',
            re.MULTILINE,
        )
        return reg_url
    ...
    def get_novel_content_reg(self):
        reg_content = re.compile(
            '<div.*?id="content".*?>(?P<content>.*?)<div.*?class.*?=.*?"bottem2">',
            re.DOTALL,
        )
        return reg_content
